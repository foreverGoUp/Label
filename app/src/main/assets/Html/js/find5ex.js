...
var find_window_height = 99; // height of window
		...
		if (node.nodeType == 3) // text node
		{
			...
			{	
				...
				if (match_pos > -1) // if we found a match
				{
					...
					var highlight_span = document.createElement("tspan"); // create a span in the middle
...
function findit() 
{
	...
	else
	{
		...
		{
			find_msg.innerHTML = "&nbsp;<b>0/0</b>"; // ver 5.1 - 10/17/2014 - changed from "Not Found"
			find_pointer = -1;	
		}
	}
	findwindow.style.visibility = 'visible';
...
// This part creates the instructions and the "find" button
	...
	+ ' onchange="resettext();" placeholder="输入查询条件后点击下一条">'
	+ '&nbsp;&nbsp;<input type="button" value="上一条" onclick="findprev();">'
	+ '&nbsp;&nbsp;<input id="btn" type="button" value="下一条" onclick="this.blur(); findit();">' // ver 5.3 - 5/15/2015 - added this.blur();
	+ '&nbsp;&nbsp;<span id="find_msg"></span>'
	+ '&nbsp;&nbsp;<input type="button" value="关闭" onclick="hide()" style="color:#fff;background:#ff0000;">'
	+ '</form></div>\n';
	// ^ ver 5.1 - 10/17/2014
...
document.write('<input type="button" value="" style="display:none;"' 
	+ ' onclick="show();">');
